...
// Home dedicada à vendedora "Yvi". Renderizada em /track_vendors/yvi.
//
// Visualmente é uma cópia 1:1 da home padrão (`src/app/(main)/page.tsx`).
// As seções (Testimonials, Faq, Pricing, etc.) são cópias editáveis em
// `./components/` — pode mexer aqui sem afetar a home padrão.
//
// Tracking: o `<TrackHandler />` lê `?track=<nome>` da URL e grava
// cookie `signup_track`. Se a pessoa cadastrar conta com cupom, o valor
// vai parar em `profiles.user_track` (ver `/api/auth/signup-trial`).
...
export default function HomePage() {
  ...
  return (
    <div className="relative bg-dark-bg text-text-secondary overflow-clip transition-colors duration-500">
      ...
      {/* Captura `?track=<nome>` da URL e grava cookie `signup_track`.
          Suspense é exigência do Next 15 pra `useSearchParams`. */}
