...
import { PROMO_ROSA_MAX } from "@/lib/capture-promo-cards";
...
const MAX_SLOT = PROMO_ROSA_MAX - 1;
...
export async function POST(req: NextRequest) {
  ...
  const slotRaw = typeof slotField === "string" ? parseInt(slotField, 10) : NaN;
  ...
  if (!Number.isFinite(slotRaw) || slotRaw < 0 || slotRaw > MAX_SLOT) {
    return NextResponse.json({ error: `slot deve ser 0 a ${MAX_SLOT}.` }, { status: 400 });
  }
  ...
  if (oldPath) {
    if (!oldPath.startsWith(`${authData.user.id}/${site.id}/promo-rosa-cards/`)) {
  ...
  const path = `${authData.user.id}/${site.id}/promo-rosa-cards/card-${slotRaw}-${ts}.${ext}`;
