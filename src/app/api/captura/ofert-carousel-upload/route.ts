...
export async function POST(req: NextRequest) {
  ...
  if (!Number.isFinite(slotRaw) || slotRaw < 0 || slotRaw > 3) {
    return NextResponse.json({ error: "slot deve ser 0, 1, 2 ou 3." }, { status: 400 });
  }
  ...
  if (oldPath) {
    if (!oldPath.startsWith(`${authData.user.id}/${site.id}/carousel/`)) {
  ...
  const path = `${authData.user.id}/${site.id}/carousel/slot-${slotRaw}-${ts}.${ext}`;
